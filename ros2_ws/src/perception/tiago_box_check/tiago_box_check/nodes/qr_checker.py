...
class QRCheckerNode(Node):
    """
    QR Checker Node (OpenCV QRCodeDetector)
    ...
    def __init__(self):
        super().__init__("qr_checker_node")  # ✅ 노드 이름 변경
        ...
        self.get_logger().info("✅ QR checker node initialized (easyocr removed).")
    ...
    def image_cb(self, msg: Image):
        ...
        if most_common[1] >= self.stability_frames and most_common[0] and most_common[0] != self.last_logged:
            stable_text = most_common[0]

            # publish raw qr text
            self.pub_qr_text.publish(String(data=stable_text))

            # (선택) destination_id publish
            if self.publish_destination_id:
